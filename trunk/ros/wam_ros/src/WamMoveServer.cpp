...
bool moveFunc(wam_ros::WamCommands::Request  &req,
         wam_ros::WamCommands::Response &res );
...
int main(int argc, char **argv)
{
	...
	/* Advertise service for moveTo function call */
	ros::ServiceServer moveService = n.advertiseService("move_cmd", moveFunc); 
         ...
         wam_ros::WamCommands::Response &res )
{
	...
	double f[7];
	for ( int i = 0; i < 7; i++)
		f[i] = req.desiredJoints[i]; 
	ROS_INFO("moving to received joint positions");
	wam.moveTo(7, f);
