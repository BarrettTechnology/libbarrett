 ...
 *    to bt_log_trigger()
 *  - a \b chunk is a buffer-sized set of records written to disk at a time
 *
 ...
 * bt_log_decode_file("datafile.bin", "datafile.csv", 1, 0);
 * \endcode
 */
...
struct bt_log
{
   ...
   int initialized; /**< Has the bt_log been initialized? */
   ...
   int chunk_idx;
   FILE * binfile;
   long binfile_dataloc;
   int finished; /**< Has the binary file been all written? */

   int records_read;
   int chunk_records_to_read;
};
 ...
 *                     (pass 0 and it will use a temporary file)
 * \retval 0 Success
 * \retval -1 bt_log already initialized
 ...
 */
int bt_log_decode_file(char * infile, char * outfile, int header,
                       int octave);
