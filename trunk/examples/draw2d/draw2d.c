...
int main(int argc, char ** argv)
{
   ...
   gsl_matrix_set(wam_local->kin->tool->trans_to_prev, 2,3, 0.22); /* marker */
      ...
      switch (key)
      {
         ...
         case 'U':
            ...
            /* Open a new one */
            {
               ...
               struct config_t cfg;
               config_init(&cfg);
               err = config_read_file(&cfg,"draw2d.config");
               if (err != CONFIG_TRUE) break;
               cdraw = control_draw2d_create(config_lookup(&cfg,"control_draw2d"),
                                              wam_local->kin, wam_local->dyn);
               config_destroy(&cfg);
            }
            if (!cdraw)
            {
               syslog(LOG_ERR,"Could not create the draw2d controller.");
               break;
            }
            bt_wam_idle(wam);
         ...
         case 'u':
            ...
            control_draw2d_destroy(cdraw);
            cdraw = 0;
            break;
         case 'i':
            if (!cdraw) break;
            control_draw2d_press(cdraw);
            break;
         case 'o':
            if (!cdraw) break;
            control_draw2d_hover(cdraw);
            break;
