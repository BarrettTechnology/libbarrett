...
void * init_wamdisplay_controller(void * param)
{
   printf("init wamdisplay_controller");
   pthread_mutex_t * m = (pthread_mutex_t *) param; 
   wamdisplay_controller gui_controller(shared.angle, m);
...
int main(int argc, char *argv[])
{
  ...
  pthread_create( &thread1, NULL, graphicThread, NULL);

  pthread_create( &thread2, NULL, comunicationThread, NULL);
  //pthread_create( &thread2, NULL, init_wamdisplay_controller, (void *) &mutex1);
      ...
      {  
		  switch(state)
		  {
			case VIEW:								
				...
				get_angles(string, 7, socket_angles);
...
void read_shared_memory(){
   pthread_mutex_lock( &mutex1 );
   wam7DOFparam[0].theta=shared.angle[0]+theta1;
   wam7DOFparam[1].theta=shared.angle[1]+theta2;
   wam7DOFparam[2].theta=shared.angle[2]+theta3;
   wam7DOFparam[3].theta=shared.angle[3]+theta4;
   wam7DOFparam[4].theta=shared.angle[4]+theta5;
   wam7DOFparam[5].theta=shared.angle[5]+theta6;
   wam7DOFparam[6].theta=shared.angle[6]+theta7;
   pthread_mutex_unlock( &mutex1 );
}
