...
template<size_t DOF>
class MasterMaster : public SingleIO<units::JointPositions<DOF>, units::JointPositions<DOF> > {
public:
...
//				}
	...
	virtual void operate() {
		...
		if (init) {
			this->outputValue->setValue(units::JointPositions<DOF>(0.0));
			return;
		}
...
//			this->outputValue->setValue(0.5*theirJp + 0.5*this->input.getValue());
	...
	bool locked;
	int numMissed;
	units::JointPositions<DOF> theirJp;
