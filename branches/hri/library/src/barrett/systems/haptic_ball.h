...
public:
	HapticBall(units::CartesianPosition center, double radius) :
		c(center), r(radius) {}
	virtual ~HapticBall() {}

protected:
	virtual void operate() {
		units::CartesianForce error = input.getValue() - c;
...
//			directionOutputValue->setValue(units::CartesianForce::Zero());
			directionOutputValue->setValue(units::CartesianForce(0.0));
		}
	}

	units::CartesianPosition c;
	double r;
