...
int mu_callback(struct bt_wam_local * wam_local)
{
   int j;
   /* Save the current joint torques for each joint */
   if (mu_n < NUM_POINTS)
   {
      for (j=0; j<wam_local->wambot->dof; j++)
      {
         mu_jts[j][mu_n] = gsl_vector_get(wam_local->jtorque,j);
         mu_jps[j][mu_n] = gsl_vector_get(wam_local->jposition,j);
      }
      mu_n++;
   }
   return 0;
...
int do_gravity(char * wamname)
{
    ...
    * (fills poses, sets num_poses) */
   {
      ...
      config_init(&cfg);
      err = config_read_file(&cfg,"/etc/wam/samples/cal.config");
      if (err != CONFIG_TRUE)
      {
   ...
   while (!done)
   {
      ...
      for (j=0; j<n; j++)
      {
         ...
         mvprintw(5, 13 + 9*j, "% 08.5f ",gsl_vector_get(wam_local->jposition,j));
         mvprintw(6, 13 + 9*j, "% 08.4f ",gsl_vector_get(wam_local->jtorque,j));
      }
   ...
   if (done == 1)
   {
      ...
      for (pose=0; pose<num_poses; pose++)
      {
         ...
         bt_kinematics_eval(wam_local->kin, positions[pose], 0);
         bt_calgrav_eval(wam_local->grav, 0); /* find g vectors */
         for (j=0; j<n; j++)
         {
            /* GT: Gravity skew matrix */
            gsl_matrix_set(GT[j], 3*pose+0, 1, -gsl_vector_get(wam_local->grav->g[j],2) );
            gsl_matrix_set(GT[j], 3*pose+0, 2,  gsl_vector_get(wam_local->grav->g[j],1) );
            gsl_matrix_set(GT[j], 3*pose+1, 0,  gsl_vector_get(wam_local->grav->g[j],2) );
            gsl_matrix_set(GT[j], 3*pose+1, 2, -gsl_vector_get(wam_local->grav->g[j],0) );
            gsl_matrix_set(GT[j], 3*pose+2, 0, -gsl_vector_get(wam_local->grav->g[j],1) );
            gsl_matrix_set(GT[j], 3*pose+2, 1,  gsl_vector_get(wam_local->grav->g[j],0) );
            /* GT: -R*L */
            gsl_matrix_set(GT[j], 3*pose+0, 3+2*pose+0, -gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,0,0) );
            gsl_matrix_set(GT[j], 3*pose+0, 3+2*pose+1, -gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,1,0) );
            gsl_matrix_set(GT[j], 3*pose+1, 3+2*pose+0, -gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,0,1) );
            gsl_matrix_set(GT[j], 3*pose+1, 3+2*pose+1, -gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,1,1) );
            gsl_matrix_set(GT[j], 3*pose+2, 3+2*pose+0, -gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,0,2) );
            gsl_matrix_set(GT[j], 3*pose+2, 3+2*pose+1, -gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,1,2) );
            /* Y */
            gsl_vector_set(Y[j], 3*pose+0, gsl_vector_get(torques[pose],j) * gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,2,0) );
            gsl_vector_set(Y[j], 3*pose+1, gsl_vector_get(torques[pose],j) * gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,2,1) );
            gsl_vector_set(Y[j], 3*pose+2, gsl_vector_get(torques[pose],j) * gsl_matrix_get(wam_local->kin->link[j]->rot_to_prev,2,2) );
            if (j < n-1)
      ...
      printf("replacing any existing calgrav groups.\n");
