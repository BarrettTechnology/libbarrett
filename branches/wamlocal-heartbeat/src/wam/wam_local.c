...
struct bt_wam_local * bt_wam_local_create_cfg(char * wamname, config_setting_t * wamconfig, enum bt_wam_opt opts)
{
   ...
   syslog(LOG_ERR, "%s: hello world!", __func__);
   ...
   wam->rt_thread = 0;
   wam->nonrt_thread = 0;
   wam->last_heartbeat_time = 0; //1e9 * bt_os_rt_get_time();
   wam->heartbeat_dead = 0;
   
   syslog(LOG_ERR, "last heartbeat time set to %f", wam->last_heartbeat_time);
   ...
   syslog(LOG_ERR, "%s: attempting to start rt thread.", __func__);
...
{  
   ...
   {
	   syslog(LOG_ERR, "%s: heartbeat dead, destroying rt thread.", __func__);
...
{
	wam->last_heartbeat_time = 1e-9 * bt_os_rt_get_time();
	return 0;
}

int bt_wam_local_check_heartbeat(struct bt_wam_local * wam)
{
	if (!wam->last_heartbeat_time)
		return 0;
	
	return ( ( ( 1e-9 * bt_os_rt_get_time() ) - wam->last_heartbeat_time ) > 2 ); /* check if difference is more than 2 seconds */ 
}
