...
int bt_wam_set_heartbeat(struct bt_wam * wam)
{
#ifndef ASYNC_ONLY
   if (!wam->caller)
      return bt_wam_local_set_heartbeat(wam->obj);
   else
#endif
   {
      int myint;
...
int bt_wam_check_heartbeat(struct bt_wam * wam)
{
#ifndef ASYNC_ONLY
   if (!wam->caller)
      return bt_wam_local_check_heartbeat(wam->obj);
   else
#endif
   {
      int myint;
