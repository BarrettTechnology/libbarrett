...
struct thread_data
{
   ...
   int * pconnected;
   int * pgoing;
   int * state;			//1- server_handler; 2- server_controller
...
{
   struct thread_data * my_data = (thread_data *)params;
   server_controller serverController(my_data->sock, my_data->pcmd_q, my_data->pconnected, my_data->pgoing, my_data->state);
   //clear();
...
{
   struct thread_data * my_data = (thread_data *) params;
   server_handler serverHandler(my_data->sock, my_data->pmutex_q, my_data->wam, my_data->pconnected, my_data->pgoing, my_data->state);
