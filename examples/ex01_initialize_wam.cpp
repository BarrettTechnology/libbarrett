...
// The file below defines Barrett's standard main() function (SMF). It helps
// you initialize Barrett's products properly without having to repeat the
// necessary code every time you make a new WAM program. See the documentation
// in barrett/standard_main_function.h for more information. The SMF will call
// the wam_main() function below once the WAM is ready to start applying
// torques.
...
// The thing to remember is that DOF is a constant. The compiler will generate
// a version of this function where DOF = 7, and a version where DOF = 4. The
// SMF will choose between the two versions based on what kind of robot is
